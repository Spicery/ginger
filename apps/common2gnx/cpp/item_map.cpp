...
ItemMap::ItemMap( const bool cstyle ) {
	...
	add( true,	 	"??", 				tokty_isntabsent, 	SSA, 		1900	);
	add( true,	 	"...",				tokty_explode,  	SSA,        250		);
	...
	add( true,		"*", 				tokty_mul, 			BSy, 		500		);
	add( true,		"**", 				tokty_pow, 			BSy, 		300		);
	add( true,		"/", 				tokty_div, 			BSy, 		400		);
	add( true,		"&&", 				tokty_absand, 		BFo, 		1700	);
	add( true,		"%)", 				tokty_fat_cparen, 	Clo, 		0		);
	add( true,		"%]", 				tokty_fat_cbracket, Clo, 		0		);
	add( true,		"%}", 				tokty_fat_cbrace, 	Clo, 		0		);
	add( true,		"+", 				tokty_add, 			BSy,		700		);
	...
	add( true,		">", 				tokty_gt, 			BSy, 		1300	);
	add( true,		">=", 				tokty_gte, 			BSy, 		1500	);
	...
	add( !cstyle,	"endrecordclass",	tokty_endrecordclass, 	Clo, 		0		);
	...
	add( true,		"mod",				tokty_mod,			BSy, 		500		);
	...
	add( !cstyle, 	"recordclass",		tokty_recordclass,	PrF,		0		);
	add( true,	 	"return",			tokty_return,		PrF, 		0		);
	add( !cstyle,	"slot",				tokty_slot,			Clo,		0		);
