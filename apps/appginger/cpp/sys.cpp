...
SysMap & SysMap::sysMap() {
	...
	static SysMap::value_type rawData[] = {
		...
		SysMaplet( "negate", SysInfo( vmc_neg, Arity( 1 ), Arity( 1 ), "Negates a number (same as multiplying by -1)" ) ),
		SysMaplet( "positivate", SysInfo( vmc_pos, Arity( 1 ), Arity( 1 ), "Checks for a number (same as multiplying by 1)" ) ),
		...
		SysMaplet( "!=^=", SysInfo( CMP_NEQ, Arity( 2 ), Arity( 1 ), "Non-identity of two values, result is boolean" ) ),
