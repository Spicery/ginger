...
void plant_term( Plant plant, Term term ) {
	...
	switch ( fnc ) {
		...
		case fnc_semi: {
			plant0( plant, term_index( term, 0 ) );
			plant_term( plant, term_index( term, 1 ) );
			break;
		}
    	case fnc_comma: {
			plant_term( plant, term_index( term, 0 ) );
			plant_term( plant, term_index( term, 1 ) );
			break;
		}
		case fnc_id: {
			...
			if ( id != NULL ) {
				...
				if ( slot >= 0 || !id->is_local ) {
					...
					mishap( "Ident record not assigned slot (%s)", term_id_string( term ).c_str() );
				}
			} else {
				mishap( "Unlifted identifier %s", term_id_string( term ).c_str() );
			}
			break;
		}
		case fnc_var: {
			Ident ident = term_id_ident( term_index( term, 0 ) );
		...
		case fnc_assign: {
			Term lhs = term_index( term, 0 );
			if ( term_is_id( lhs ) ) {
				Ident ident = term_id_ident( lhs );
				Term rhs = term_index( term, 1 );
				plant1( plant, rhs );
				vmiPOPID( plant, ident );
			} else {
				reset( "Not implemented yet (%s)", term_functor( lhs ) );
			}
			break;
		}
		case fnc_fn: {
		...
		case fnc_app: {
			...
			if ( term_is_id( fn ) ) {
				if ( aargs == DONTKNOW ) {
					...
					vmiEND_CALL_ID( plant, v, term_id_ident( fn ) );
				} else {
					plant_term( plant, args );
					vmiSET_CALL_ID( plant, aargs, term_id_ident( fn ) );
				}
...
void plant1( Plant plant, Term term ) {
	...
	if ( a == DONTKNOW ) {
		...
		reset( "%d results in single context", a );
	}
}
...
void plant0( Plant plant, Term term ) {
	...
	if ( a == DONTKNOW ) {
		...
		reset( "%d results in zero context", a );
	}
}
