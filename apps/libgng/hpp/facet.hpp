...
public:
	...
	bool isEmptyIntersection( const FacetSet * fs ) const;
	const FacetSet * add( const std::string & f ) const;
	const FacetSet * add( const Facet * f ) const;
	const FacetSet * addSet( const FacetSet * fs ) const;
	bool isEmpty() const;
